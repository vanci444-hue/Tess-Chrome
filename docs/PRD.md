...
| 版本 | v4.0 |
| 状态 | Confirmed；2026-09-20 用户确认整体方案 |
...
**选择或创建客户 → 新建或继续本次报告会话 → 官网 Capture → 实时语音转文字并手动发送 → 确认关键疑点及业务追问 → Agent 按需查询、计算并准备 Report → 销售审核 → 生成链接并打开 Report → 查看模拟客户动态并继续跟进。**

销售无需逐项启动地图查询、金融规划和成本计算。原有六个 Case 保留为同一销售工作流中的能力环节，不要求每个客户都执行全部分析。
...
| CRM 信息录入 | 销售手动复制并粘贴必要文本，Agent 提取后由销售确认；不自动访问真实 CRM |
| 车型采集 | 从真实官网页面读取当前选中的方案 |
| 销售复盘 | 真实实时 ASR，边说边显示文字；销售可纠正，手动点击发送后 Agent 才处理；也支持直接文字输入 |
| 充电分析 | 真实接入高德，查询区域、站点、距离及路线 |
...
| Report | 根据本次客户信息生成，审核后提供演示电脑可打开的链接 |
| Report 内部按钮 | Ask Tess、分享、联系与预约等业务按钮静态；地图图片及“在高德继续搜索”可真实打开外部地图 |
...
销售通过实时语音转写或文字描述试驾情况。录音期间持续显示识别文字；停止后等待本段识别结束，文字留在输入框供检查和纠正，必须由销售手动点击“发送”，才提交到本次会话并触发 Agent 理解。停止录音、识别完成、切换页面均不等于发送。

实时识别需要向 ASR 服务传送音频；“未发送”指尚未作为会话消息交给业务 Agent，不代表音频未离开电脑。ASR 中间结果不得直接写入已确认 Context、触发查询或生成报告。发送后系统再提取人物关系、偏好、反馈、预算、里程、充电条件及未决问题。
...
- **AC-017**：Ask Tess、分享、联系销售等按钮仅作静态展示，不触发真实业务操作。
...
客户列表提供“新增客户”。新客户可手填或粘贴 CRM 文本，由 Agent 提取称呼、联系方式和已有背景，销售确认并补齐必填项后创建客户和本次报告会话。CRM 文本保留来源性质，不要求先整理成固定格式。

手机号或邮箱相同仅触发轻量提醒，不按昵称查重、不自动合并档案。销售可进入已有客户新建报告；若确认是不同的人，则允许继续创建。已有客户默认打开最近会话，可继续未完成复盘，或点击“新建试驾报告”；历史报告收在独立入口，不挤占主操作区域。
...
- **AC-023**：手机号或邮箱相同时提示已有客户并允许进入；同昵称不同联系方式不触发身份合并；销售明确选择不同客户时仍可创建独立档案。
...
整体保持 Tesla 与 ChatGPT 的简洁、克制风格。

- 黑白灰为主，颜色用于必要状态与图表区分。
- 插件以对话、工具结果和下一步操作为主。
- 录音时显示实时文字，停止后文字留在输入框，销售检查并点击发送；录音按钮和发送按钮含义清晰，不把停止动作做成自动发送。
- 窄侧栏一次只显示客户列表、客户会话或历史报告列表中的一个主视图，不把三者并排常驻；进入会话后收起客户列表。
- 模拟已登录销售身份仅作展示；会话顶部保留客户称呼、返回／切换客户及历史报告入口。历史报告可直接查找，不强迫翻聊天；对话内仍保留对应 Report 卡片。
- 完整 Report 在 Chrome 新标签页展示；本次不做登录页或完整 CRM。具体尺寸、配色和控件排布留给界面设计。
- Report 以车辆图片、关键数字、方案对比和易读图表为主。
- 充电地图展示真实静态结果，明确提供“在高德继续搜索”，点击新标签页打开；其余业务按钮保持演示性质。
- 先展示结论，再展示来源、计算条件和待确认事项。
- 不使用缺乏依据的雷达评分、购买意愿分数或装饰性仪表盘。

## Demo 整体验收

- 走通手填或 CRM 粘贴建档、真实 Capture、边说边出字及销售手动发送、关键确认、按需工具调用、审核及本机 Report 链接的完整案例。
...
2026-09-20，用户明确回复“确认”，认可本版需求、技术方案及原音频不落盘建议，授权进入侧栏与 Report 界面原型。需求和技术方案标为 Confirmed；真实服务仍需开发联调，界面原型完成后另行整体审阅。
