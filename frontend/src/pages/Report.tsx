import { useEffect, useState } from "react";
import { useParams } from "react-router";
import { tess } from "../services/tess";
import type { ReportSnapshot } from "../types/api";
import { ErrorNotice, Loading, ModeBanner } from "../components/Shared";
import { date } from "../utils/display";
import { moduleData, optionsFrom, safeOfficialUrl } from "../utils/reportData";
import {
  OptionsModule,
  FinanceModule,
  EnergyModule,
  ChargingModule,
  FamilyModule,
  TrialModule,
  MissingModule,
  StaticAsk,
} from "../components/ReportModules";
import {
  matterLines,
  readDemoDraft,
  resolvedLines,
  tradeoffLines,
} from "../mocks/reportScene";
import "../report.css";

type ReportView = "web" | "phone";

export default function Report({ preview = false }: { preview?: boolean }) {
  const { reportId, sessionId } = useParams();
  // Each report owns its loading and map states; never show the prior customer's snapshot.
  return (
    <ReportContent
      key={`${preview ? "preview" : "report"}:${reportId || sessionId}`}
      preview={preview}
    />
  );
}

function ReportContent({ preview }: { preview: boolean }) {
  const { reportId = "", sessionId = "" } = useParams(),
    [data, setData] = useState<ReportSnapshot | null>(null),
    [error, setError] = useState(""),
    [stale, setStale] = useState(false),
    [view, setView] = useState<ReportView>(() => {
      try {
        return localStorage.getItem("tess.demo.report.view") === "phone"
          ? "phone"
          : "web";
      } catch {
        return "web";
      }
    });

  useEffect(() => {
    let alive = true;
    const load = async () => {
      if (preview) {
        try {
          const s = await tess.session(sessionId);
          if (!s.draft) throw new Error("本次会话尚无可预览草稿");
          if (alive)
            setStale(s.draft.stale || s.draft.source_revision !== s.revision);
          return s.draft.report_data;
        } catch (first) {
          const local = readDemoDraft(sessionId);
          if (local?.report_data) {
            if (alive) setStale(false);
            return local.report_data;
          }
          throw first;
        }
      }
      return tess.report(reportId);
    };
    load()
      .then((r) => {
        if (alive) setData(r);
      })
      .catch((e) => {
        if (alive) setError(e.message);
      });
    return () => {
      alive = false;
    };
  }, [reportId, sessionId, preview]);

  function switchView(next: ReportView) {
    setView(next);
    try {
      localStorage.setItem("tess.demo.report.view", next);
    } catch {
      /* ignore */
    }
  }

  const options = data ? optionsFrom(data.modules) : [],
    advisorModule = data?.modules.find((m) => m.type === "advisor"),
    advisor = advisorModule
      ? moduleData<{
          id: string;
          name: string;
          store_name: string;
          is_demo: boolean;
        }>(advisorModule)
      : null;
  const tradeoffs = data ? tradeoffLines(data.summary) : [];
  const matters = data ? matterLines(data.summary) : [];
  const resolved = data ? resolvedLines(data.summary) : [];
  return (
    <div className={`report-shell layout-${view}`}>
      <div className="report-view-chrome">
        <div className="report-view-tabs" role="tablist" aria-label="报告视图">
          <button
            type="button"
            role="tab"
            className={view === "web" ? "is-active" : ""}
            aria-selected={view === "web"}
            onClick={() => switchView("web")}
          >
            Web
          </button>
          <button
            type="button"
            role="tab"
            className={view === "phone" ? "is-active" : ""}
            aria-selected={view === "phone"}
            onClick={() => switchView("phone")}
          >
            Phone
          </button>
        </div>
      </div>
      <main className={`report-page is-${view}`}>
        <ModeBanner />
        <header className="report-topbar">
          <span className="wordmark">Tess</span>
          <span>TEST DRIVE REPORT</span>
          <span>{preview ? "草稿预览" : "试驾后的下一步"}</span>
        </header>
        <ErrorNotice message={error} />
        {!data && !error && <Loading />}
        {data && (
          <>
            {preview && (
              <div className={`notice ${stale ? "warning" : ""}`}>
                {stale
                  ? "草稿待更新，不能发布。请回销售侧重新准备。"
                  : "销售审核预览 · 尚未发布，不代表最终报告。"}
              </div>
            )}
            <header className="report-hero">
              <p className="eyebrow">{date(data.generated_at)} · MODEL Y</p>
              <p className="hero-greeting">{data.customer_salutation}</p>
              <h1>这一程，值得被记住。</h1>
              <p className="hero-subtitle">
                今天开过的记忆都还在；方向盘上的念头，已帮您整理好。
              </p>
              <div className="three-questions">
                <section>
                  <span>01 / Your Shortlist</span>
                  <h2>现在比较什么</h2>
                  <p>{data.summary.comparing || "候选方案仍待确认"}</p>
                </section>
                <section>
                  <span>02 / Resolved</span>
                  <h2>已经明确什么</h2>
                  {data.summary.confirmed.length ? (
                    <ul>
                      {data.summary.confirmed
                        .filter(
                          (x) =>
                            !x.startsWith("权衡：") && !x.startsWith("权衡:"),
                        )
                        .map((x, i) => (
                          <li key={i}>{x}</li>
                        ))}
                    </ul>
                  ) : (
                    <p>暂无已确认结论。</p>
                  )}
                </section>
                <section>
                  <span>03 / Still to Decide</span>
                  <h2>还需确认什么</h2>
                  {data.summary.pending.length ? (
                    <ul>
                      {data.summary.pending.map((x, i) => (
                        <li key={i}>{x}</li>
                      ))}
                    </ul>
                  ) : (
                    <p>本次暂未记录新的待确认事项。</p>
                  )}
                </section>
              </div>
            </header>
            {(matters.length > 0 || resolved.length > 0) && (
              <section className="report-section" id="matters">
                <div className="section-heading">
                  <p className="eyebrow">WHAT MATTERS TO YOU</p>
                  <h2>对你真正重要的事</h2>
                  <p>来自今日已确认的偏好与顾虑，不是模型猜测。</p>
                </div>
                {matters.length > 0 && (
                  <ul className="report-matter-list">
                    {matters.map((line) => (
                      <li key={line}>{line}</li>
                    ))}
                  </ul>
                )}
                {resolved.length > 0 && (
                  <>
                    <h3 className="report-subhead">Questions Resolved Today</h3>
                    <ul className="report-matter-list">
                      {resolved.map((line) => (
                        <li key={line}>{line}</li>
                      ))}
                    </ul>
                  </>
                )}
              </section>
            )}
            {tradeoffs.length > 0 && (
              <section className="report-section" id="tradeoffs">
                <div className="section-heading">
                  <p className="eyebrow">KEY TRADE-OFFS</p>
                  <h2>真正影响决策的取舍</h2>
                  <p>在支付能力与偏好之间，今天需要看清的差别。</p>
                </div>
                <ul className="report-matter-list">
                  {tradeoffs.map((line) => (
                    <li key={line}>{line}</li>
                  ))}
                </ul>
              </section>
            )}
            {data.modules
              .filter((m) => m.type === "trial")
              .map((m, i) => (
                <TrialModule module={m} key={`trial-${i}`} />
              ))}
            {options.length > 0 && (
              <OptionsModule options={options} modules={data.modules} />
            )}
            <div className="report-modules">
              {data.modules
                .filter((m) => !["options", "trial", "advisor"].includes(m.type))
                .map((m, i) => {
                  if (
                    m.type === "charging" &&
                    Array.isArray(m.data.stations) &&
                    typeof m.data.region === "string"
                  )
                    return (
                      <ChargingModule
                        key={i}
                        module={m}
                        owner={preview ? sessionId : reportId}
                        preview={preview}
                      />
                    );
                  if (m.status === "missing" || m.type === "missing")
                    return <MissingModule module={m} key={i} />;
                  switch (m.type) {
                    case "finance":
                      return (
                        <FinanceModule key={i} module={m} options={options} />
                      );
                    case "energy":
                      return <EnergyModule key={i} module={m} />;
                    case "charging":
                      return (
                        <ChargingModule
                          key={i}
                          module={m}
                          owner={preview ? sessionId : reportId}
                          preview={preview}
                        />
                      );
                    case "family":
                      return <FamilyModule key={i} module={m} />;
                    default:
                      return <MissingModule key={i} module={m} />;
                  }
                })}
            </div>
            <section className="report-section ask-section">
              <div className="section-heading">
                <p className="eyebrow">NEXT ACTIONS</p>
                <h2>从这次试驾，继续下一步。</h2>
                <p>
                  可以基于当前候选与待确认事项继续提问，或预约再次到店。当前页面的问答、分享和联系按钮仅为
                  Prototype 展示。
                </p>
              </div>
              <div className="ask-grid">
                <StaticAsk>Ask Tess · 家充装不了怎么办？</StaticAsk>
                <StaticAsk>Contact Advisor · 联系销售</StaticAsk>
                <StaticAsk>Schedule Test Drive · 再约试驾</StaticAsk>
                <StaticAsk>Visit Store · 预约到店</StaticAsk>
                <StaticAsk>Order · 下定（演示）</StaticAsk>
              </div>
            </section>
            <section className="advisor-section">
              <div>
                <p className="eyebrow">YOUR SALES ADVISOR</p>
                <h2>{advisor?.name || "销售顾问信息待确认"}</h2>
                <p>{advisor?.store_name || "门店信息待确认"}</p>
                {advisor?.is_demo && (
                  <span className="tag">Demo · 模拟销售身份</span>
                )}
              </div>
              <div>
                <p>
                  如果某个问题还不能可靠回答，可以带着这次试驾的背景继续与销售确认。
                </p>
                <div className="row wrap">
                  <button disabled>联系销售 · 演示</button>
                  <button disabled>分享给家人 · 演示</button>
                </div>
              </div>
            </section>
            <footer className="report-sources">
              <h3>信息来源与时间</h3>
              <p>
                报告生成：{date(data.generated_at)}
                {data.published_at ? ` · 发布：${date(data.published_at)}` : ""}
              </p>
              {data.sources.map((source) => (
                <div className="source-row" key={source.id}>
                  <span>
                    {source.label} · {source.kind}
                  </span>
                  <span>{date(source.observed_at)}</span>
                  {source.url && safeOfficialUrl(source.url) && (
                    <a
                      href={safeOfficialUrl(source.url)!}
                      target="_blank"
                      rel="noreferrer"
                    >
                      官方来源 ↗
                    </a>
                  )}
                </div>
              ))}
              <p>{data.disclaimer}</p>
              <p>
                价格、金融、权益、交付和站点状态可能变化。本页保留原采集时间，不自动刷新为当前有效信息。
              </p>
              <span className="report-signoff">tess · A clearer next step.</span>
            </footer>
          </>
        )}
      </main>
    </div>
  );
}
